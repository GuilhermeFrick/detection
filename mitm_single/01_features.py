"""
Extracao de features para deteccao de MITM Single SOME/IP (mitm_single_attacker).

Le benign_traffic.csv + mitm_single_attacker.csv e extrai 12 features focadas
nas caracteristicas do ataque MITM por atacante unico (SD Withdraw + ADAS Injection):
    ...
    f21  is_relay_service        -- 1 se service_id=0x100B (nao usado neste cenario)

Diferenca vs mitm_multi:
    Sem componente relay (sem service_id=0x100B) -> f21 sempre 0
    SD Withdraw enviado pelo IP real do atacante (172.18.0.13) -> label=1 + f19=1
    Discriminadores principais: f19 + f20 + f17/f18

Saida:
    mitm_single/data/features.csv
    mitm_single/data/X_train.npy  y_train.npy
    mitm_single/data/X_test.npy   y_test.npy

Uso:
    python mitm_single/01_features.py
"""
import sys, time
import numpy as np
import pandas as pd
...
PCAP_FILES = [
    ...
    PARSED_DIR / 'mitm_single_attacker.csv',
]
...
def extract(df: pd.DataFrame, state: dict) -> pd.DataFrame:
    ...
    # f21: 1 se service_id == 0x100B (relay service — nao esperado neste cenario)
    f21 = np.zeros(n)
    for i in range(n):
    ...
    for p in PCAP_FILES:
        if not p.exists():
            raise FileNotFoundError(
                ...
                f'  python src/01_parse.py --pcaps benign_traffic.pcap mitm_single_attacker.pcap'
            )
