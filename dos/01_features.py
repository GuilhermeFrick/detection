"""
Extração de features para detecção de DoS SOME/IP.

Lê benign_traffic.csv + dos_noti_flood.csv e extrai features comportamentais
focadas nas características do ataque DoS (flood de NOTIFICATION messages):

    f01  ip_time_interval      — intervalo entre pacotes do mesmo fluxo
    f08  someip_payload_change — distância Hamming entre payloads consecutivos
    f11  ip_length_change      — variação do tamanho IP entre pacotes do fluxo
    f12  tcpudp_length_change  — variação do tamanho TCP/UDP
    f13  payload_repeat_rate   — fração dos últimos 5 payloads idênticos ao atual
    f15  someip_payload_len    — comprimento do payload SOME/IP
    f16  tcpudp_len            — comprimento da camada transporte
    f17  src_packet_rate       — pacotes/s deste src_ip (janela 1000 pacotes)
    f18  src_payload_diversity — payloads únicos / total deste src_ip (janela)

As features f02-f07 (byte distribution) foram omitidas: o DoS é detectável
com features comportamentais simples, sem necessidade de modelos de bytes.

Saída:
    dos/data/features.csv      — dataset completo com label
    dos/data/X_train.npy  y_train.npy
    dos/data/X_test.npy   y_test.npy

Uso:
    python dos/01_features.py
"""
import sys, time
import numpy as np
import pandas as pd
...
PCAP_FILES = [
    ...
    PARSED_DIR / 'dos_noti_flood.csv',
]


# ── Helpers de estado de fluxo ────────────────────────────────────────────────
...
def extract(df: pd.DataFrame, state: dict) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        # f17, f18 — taxa e diversidade por src_ip
...
if __name__ == '__main__':
    ...
    for p in PCAP_FILES:
        if not p.exists():
            raise FileNotFoundError(
                f'{p} não encontrado — rode primeiro:\n'
                f'  python src/01_parse.py --pcaps benign_traffic.pcap dos_noti_flood.pcap'
            )

    if FEAT_CSV.exists():
        print(f'[OK] {FEAT_CSV} já existe — apague para re-extrair.')
    else:
        state   = make_state()
        first   = True
        n_total = 0
    ...
    print(f'  DoS     : {n1:>10,}  ({100*n1/len(labels):.1f}%)')
    ...
    for c in FEAT_COLS:
        print(f'  {c:<30}  min={stats[c]["min"]:>12.4f}  max={stats[c]["max"]:>12.4f}')
    ...
    def export(split_name, mask):
        ...
        print(f'  {split_name}: {X.shape}  benigno={n0:,}  dos={n1:,}')
