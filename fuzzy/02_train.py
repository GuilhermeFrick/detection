"""
Treinamento do classificador Fuzzy SOME/IP.

Uso:
    python fuzzy/02_train.py
"""
import sys, time
import numpy as np
...
if __name__ == '__main__':
    ...
    print(f'  Treino : {X_train.shape}  (benigno={(y_train==0).sum():,}  fuzzy={(y_train==1).sum():,})')
    print(f'  Teste  : {X_test.shape}   (benigno={(y_test==0).sum():,}   fuzzy={(y_test==1).sum():,})')
    ...
    print('\n' + '='*50)
    print('  RESULTADOS -- Fuzzy vs Benigno')
    print('='*50)
    ...
    path = MODEL_DIR / 'fuzzy_classifier.json'
